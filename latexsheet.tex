...
\section{Lebesgue-related}
\textbf{Lebesgue Integral}$E(X)=\int_{\Omega} X(\omega)P(d\omega)=\int_{R}xdF(x)$\\
\textbf{Lebesgue Program - Simple function} $S_m(\omega)=\sum_{n=0}^{m2^m}\frac{n}{2^m}1_{A_{mn}}(\omega)$\\
\textit{(Properties)} Functions are \textit{positive}, \textit{increasing}, and \textit{converges pointwise} to $X$, $S_m(\omega)\leftarrow X(\omega)$ for each $\omega$\\
\textbf{Stirling's Formula} $n!\sim \sqrt{2\pi}n^{n+1/2}e^{-n}$ $(n \rightarrow \infty)$
\end{multicols*}
\end{document}
